package cn.cerc.sample.services.bc;
...
public class SvrSale extends CustomService {
	...
	public boolean querySale() {
		...
		if (headIn.hasValue("SearchText_")) {
			f.byLink(new String[] { "h.CusName_", "h.AppUser_" },
	...
	public boolean saveSale() {
		...
		String cusName = headIn.getString("CusName_");
		...
		if (!tb.equals("BC")) {
			throw new RuntimeException("你输入的不是销售单号！");
		} else {
	...
	public boolean findTranB() {
		...
		ds.open();
		if (ds.eof()) {
			throw new RuntimeException(String.format("单号 %s 下不存在商品编号 %s", tbNo,
					code));
		}
	...
	public boolean saveTranB() {
		...
		if (num < 0) {
			throw new RuntimeException("出货数量不能为负数！");
		}
		...
		SqlQuery dsB = new SqlQuery(this);
		dsB.add("select * from %s where TBNo_='%s' and Code_='%s'", "tranb",
				tbNo, code);
		dsB.open();
		// 更新商品库存
		if (dsPart.getDouble("Stock_") >= num) {
			dsPart.edit();
			dsPart.setField("Stock_", dsPart.getDouble("Stock_") + (-1) * num);
			dsPart.post();
			if (dsB.eof()) {
				dsB.append();
			...
			throw new RuntimeException(String.format("商品编号 %s 库存不足！还剩%s件",
					code, dsPart.getDouble("Stock_")));
		}
		return true;
	}
	...
	public boolean saveUpDateTranB() {
		...
		ds.add("select * from %s where TBNo_='%s' and Code_='%s'  ", "tranb",
				tbNo, code);
		ds.open();
		if (dsPart.getDouble("Stock_") + ds.getDouble("Num_") >= num) {
			// 更新商品库存
			dsPart.edit();
			dsPart.setField("Stock_",
					dsPart.getDouble("Stock_") + ds.getDouble("Num_") + (-1)
							* num);
			dsPart.post();
			ds.edit();
			...
			throw new RuntimeException(String.format("商品编号 %s 库存不足！还剩%s件",
					code, dsPart.getDouble("Stock_") + ds.getDouble("Num_")));
		}
		return true;
	}
